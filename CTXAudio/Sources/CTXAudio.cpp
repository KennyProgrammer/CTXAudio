...
#define CTX_LOG(x) std::cout << "[CTX Audio]  " << x << std::endl
		...
		AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
		{
			...
			if (s_DebugLog)
			{
				CTX_LOG("File Info - " << filename << ":");
				CTX_LOG("  Channels: " << channels);
				CTX_LOG("  Sample Rate: " << sampleRate);
				CTX_LOG("  Expected size: " << bufferSize);
			}
			...
			if (alGetError() != AL_NO_ERROR)
				CTX_LOG("Failed to setup sound source");
		...
		AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
		{
			...
			if (s_DebugLog)
			{
				...
				CTX_LOG("  Channels: " << channels);
				CTX_LOG("  Sample Rate: " << sampleRate);
				CTX_LOG("  Size: " << size << " bytes");

				auto [mins, secs] = result.GetLengthMinutesAndSeconds();
				CTX_LOG("  Length: " << mins << "m" << secs << "s");
			}
		...
		static void PrintAudioDeviceInfo()
		{
			if (s_DebugLog)
			{
				CTX_LOG("Audio Device Info:");
				CTX_LOG("  Name: " << s_AudioDevice->DeviceName);
				CTX_LOG("  Sample Rate: " << s_AudioDevice->Frequency);
				CTX_LOG("  Max Sources: " << s_AudioDevice->SourcesMax);
				CTX_LOG("    Mono: " << s_AudioDevice->NumMonoSources);
				CTX_LOG("    Stereo: " << s_AudioDevice->NumStereoSources);
			}
		}
